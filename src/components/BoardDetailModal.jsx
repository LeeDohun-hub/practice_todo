    ...
    <div onClick={onClose} className="fixed inset-0 flex items-center justify-center bg-black/70 bg-opacity-50 z-50">
      <div onClick={(e) => e.stopPropagation()} className="bg-white rounded-lg shadow-lg p-6 w-[600px]">
        ...
        <button onClick={handleDelete}
          className="cursor-pointer mt-4 bg-red-500 text-white px-4 py-2 rounded hover:bg-blue-600"
        >
          삭제
        </button>
        </div>
      ...
      {isOpen && <ControllerEditModal item={item} parentClose={onClose} onClose={() => setIsOpen(false)}></ControllerEditModal>}
    </div>
  );
};
